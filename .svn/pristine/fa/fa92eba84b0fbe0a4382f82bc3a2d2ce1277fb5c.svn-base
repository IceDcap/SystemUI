...
import com.android.systemui.gionee.statusbar.GnNetworkSpeedController;
import com.android.systemui.gionee.statusbar.GnSkyLightStatusBar;
...
//    KeyguardStatusBarView mKeyguardStatusBar;
    ...
    private View mNetworkSpeedView;
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        mMoreDotIcon = mStatusBarView.findViewById(R.id.moreIcon_dot);
        mNotificationIcons.setOverflowIndicator(mMoreDotIcon);
...
//        mStatusIconsKeyguard = (LinearLayout) mKeyguardStatusBar.findViewById(R.id.statusIcons);
        ...
        TextView percentTextView = (TextView) mStatusBarWindow.findViewById(R.id.percentage);
        mBatteryController.addLabelView(percentTextView);
        TextView batteryTextView = (TextView) mStatusBarWindow.findViewById(R.id.battery_txt);
        mBatteryController.addLabelView(batteryTextView);
...
//        mKeyguardStatusBar.setBatteryController(mBatteryController);
    ...
    private void initNotificationHeader() {
    	createNotificationHeaders();
    	mStackScroller.addView(mImportantHeader);
    	mStackScroller.addView(mOtherHeader);
    }
    
    protected void createNotificationHeaders() {
    	mImportantHeader = (GnNotificationLevelHeader)LayoutInflater.from(mContext).
    			inflate(R.layout.gn_notification_header, mStackScroller, false);
    	TextView important = (TextView)mImportantHeader.findViewById(R.id.header_text);
    	important.setText(R.string.important);
    	ImageView importantClearButton = (ImageView)mImportantHeader.findViewById(R.id.clear);
    	importantClearButton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View arg0) {
				clearNotifications(NotificationType.IMPORTANT);
			}
		});
    	
    	mOtherHeader = (GnNotificationLevelHeader)LayoutInflater.from(mContext).
    			inflate(R.layout.gn_notification_header, mStackScroller, false);
    	TextView other = (TextView)mOtherHeader.findViewById(R.id.header_text);
    	other.setText(R.string.other);
    	ImageView otherClearButton = (ImageView)mOtherHeader.findViewById(R.id.clear);
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (targetList.contains(child)) {
				if (mStackScroller.canChildBeDismissed(child)) {
					// If not all notifications can show in the list, notifications may not be cleared
...
//        mHandler.sendEmptyMessage(MSG_UPDATE_NOTIFICATION_HEADER);
	...
	/**
	 * Clear a message at one time
	 * */
	private void clearNotification(StatusBarNotification notification) {
		try {
			final String _pkg = notification.getPackageName();
			final String _tag = notification.getTag();
			final int _id = notification.getId();
			final int _userId = notification.getUserId();
			mBarService.onNotificationClear(_pkg, _tag, _id, _userId);
		} catch (Exception e) {
			Log.v(TAG, "RemoteView exception happened while remove notification");
		}
	}
    ...
    private void performDismissAllAnimations(ArrayList<View> hideAnimatedList) {
        Runnable animationFinishAction = new Runnable() {
            @Override
            public void run() {
                // Do not hide notification panel when clear notifications
                //animateCollapsePanels(CommandQueue.FLAG_EXCLUDE_NONE);
            }
        };
...
//        mStatusIconsKeyguard.removeViewAt(viewIndex);
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking) {
        ...
        if (!mOnConfigChanged && mUseHeadsUp && shouldInterrupt(notification)) {
...
//                if (mState == StatusBarState.SHADE) {
    ...
    protected void updateNotificationHeaders(){
    	Log.v(TAG, "updateNotificationHeaders");
    	int numChildren = mStackScroller.getChildCount();

    	boolean important = false;
    	boolean other = false;
    	ArrayList<ExpandableNotificationRow> importantList = mGnNotificationService
				.getNotificationRow(NotificationType.IMPORTANT);
    	ArrayList<ExpandableNotificationRow> otherlist = mGnNotificationService
				.getNotificationRow(NotificationType.OTHER);
    	
    	// GIONEE <wujj> <2015-02-10> modify for CR01446315 begin
    	if (importantList == null || otherlist == null) {
    		return;
    	}
    	// GIONEE <wujj> <2015-02-10> modify for CR01446315 end
    	
		for (int i = 0; i < numChildren; i++) {
			final View child = mStackScroller.getChildAt(i);
			if (importantList.contains(child) && important == false) {
				if (mStackScroller.canChildBeDismissed(child)) {
					if (child.getVisibility() == View.VISIBLE) {
						important = true;
					}
				}
			} else if (otherlist.contains(child) && other == false) {
				if (mStackScroller.canChildBeDismissed(child)) {
					if (child.getVisibility() == View.VISIBLE) {
						other = true;
					}
		...
		if (importantList.size() == 0){
			mImportantHeader.setVisibility(View.GONE);
		} else {
			mImportantHeader.setVisibility(View.VISIBLE);
			ImageView button = (ImageView)mImportantHeader.findViewById(R.id.clear);
			button.setVisibility(important ? View.VISIBLE : View.GONE);
	...
	private void gnUpdateNotificationShade() {
		...
		if (isCollapsing()) {
			addPostCollapseAction(new Runnable() {
				@Override
				public void run() {
					gnUpdateNotificationShade();
				}
			});
			return;
		}
		*/
		// GIONEE <wujj> <2015-03-02> modify for CR01448583 end

		/**
		 * Process of update notifications: 
		 * Step 1: sort notifications by ONGOING/Important/Other
		 * Step 2: remove inactive notifications
		 * Step 3: add active notifications to mStackScroll
		 * Step 4: sort notification views
		 * Step 5: insert Important and other headers in right place Step
		 * 6: update StackScroll views
		 * */

		// Step 1
		mGnNotificationService.sortNotificationViews(mNotificationData);
		ArrayList<ExpandableNotificationRow> onGoingView = mGnNotificationService
				.getNotificationRow(NotificationType.ONGOING);
		ArrayList<ExpandableNotificationRow> importantView = mGnNotificationService
				.getNotificationRow(NotificationType.IMPORTANT);
		ArrayList<ExpandableNotificationRow> otherView = mGnNotificationService
				.getNotificationRow(NotificationType.OTHER);

		final int ONGOING = onGoingView.size();
		...
		for (int i = 0; i < mStackScroller.getChildCount(); i++) {
			...
			// GIONEE <wujj> <2015-02-14> Modify for CR01445765 begin
			if (child instanceof ExpandableNotificationRow) {
				String pkgName = ((ExpandableNotificationRow) child)
						.getStatusBarNotification().getPackageName();
				if (mGnNotificationService.isInChangedList(pkgName)) {
					toRemove.add(child);
					continue;
				}
			}
		...
		if (mImportantHeader.getParent() == null) {
			mStackScroller.addView(mImportantHeader);
		}
		if (mOtherHeader.getParent() == null) {
			mStackScroller.addView(mOtherHeader);
		}
		...
		int scrollViewCount = mStackScroller.getChildCount();
		int j = 0;
		for (int i = 0; i < scrollViewCount; i++) {
			...
			if (j < ONGOING) {
				if (child == onGoingView.get(j)) {
					j++;
					continue;
				}
				mStackScroller.changeViewPosition(onGoingView.get(j), i);
				j++;
				continue;
			}

			//adjust important notifications
			if (j >= ONGOING && j < (ONGOING + IMPORTANT)) {
				int impIndex = j - ONGOING;
				if (child == importantView.get(impIndex)) {
					j++;
					continue;
				}

				mStackScroller.changeViewPosition(importantView.get(impIndex),i);
				j++;
				continue;
			}
		...
		for (int i = 0; i < scrollViewCount; i++) {
			...
			if (v instanceof ExpandableNotificationRow) {
				if (IMPORTANT > 0 && v == importantView.get(0)) {
					mImportantHeader.setVisibility(View.VISIBLE);
					if (i > 0) {
						mStackScroller.changeViewPosition(mImportantHeader, i-1);
					} else {
					...
					if (i > 0) {
						...
						mStackScroller.changeViewPosition(mOtherHeader, i);
					}
					break;
				}
			}
		...
		// the user intends to use the lock screen user switcher, QS in not
		// needed.
//		mNotificationPanel.setQsExpansionEnabled(isDeviceProvisioned()
//						&& (!mUserSwitcherController.isSimpleUserSwitcher() || mUserSetup));
		//TODO: If QS is removed from notifcationpanelView, then we should disable it to prevent handling touch/click event
    ...
    protected void updateNotifications() {
        ...
        if (GnNotificationService.SHOW_GNSTYLE_NOTIFICATION){
        	gnUpdateNotificationShade();
        } else {
        	updateNotificationShade();
        }
    ...
    protected void updateCarrierLabelVisibility(boolean force) {
        ...
        // GIONEE <wujj> <2015-02-27> modify for CR01440365 begin
		     //jiating modify for keyguard begin
        mCarrierLabel.setVisibility(View.VISIBLE);
        /*
    ...
    // GIONEE <wujj> <2015-02-27> modify for CR01440365 begin
    protected void updateCarrierLabelVisibility(int top) {
    	if (!mShowCarrierInPanel) return;
    	if (mCarrierLabel != null) {
    		mCarrierLabel.setTop(top);
    	}
    }
    // GIONEE <wujj> <2015-02-27> modify for CR01440365 begin
...
//        }
    ...
    public boolean isFalsingThresholdNeeded() {
    	//jiating modify for keyguard begin
        boolean onKeyguard = false;//getBarState() == StatusBarState.KEYGUARD;
      //jiating modify for keyguard end
...
//        if (SPEW) 
    ...
    public void animateExpandNotificationsPanel() {
        if (/*SPEW*/true) Log.d(TAG, "animateExpand: mExpandedVisible=" + mExpandedVisible);
        if (!panelsEnabled()) {
            return ;
        }
...
//        showBouncer();
    ...
    public void setWindowState(int window, int state) {
        ...
        if (mStatusBarWindow != null
            ...
            //jiating modify for keygurad begin
    ...
    public void startActivityDismissingKeyguard(final Intent intent, boolean onlyProvisioned,
        ...
        dismissKeyguardThenExecute(new OnDismissAction() {
            ...
            public boolean onDismiss() {
            	// GIONEE <wujj> <2015-03-30> modify for CR01455791 begin
            	// Sometimes, AsyncTask might not run when click notification
                //AsyncTask.execute(new Runnable() {
            	mExecutor.execute(new Runnable() {
            	// GIONEE <wujj> <2015-03-30> modify for CR01455791 end
                    public void run() {
                        try {
...
//        mNotificationPanel.setTouchDisabled(false);
    ...
    private void addNetworkSpeedView() {
        mGnNetworkSpeedController = new GnNetworkSpeedController(mContext);

        mNetworkSpeedView = (View) mStatusBarWindow.findViewById(R.id.network_speed_view);
        mNetworkSpeedView.setVisibility(View.VISIBLE);

        TextView networkSpeedTextView = (TextView) mNetworkSpeedView.findViewById(R.id.networkspeed);

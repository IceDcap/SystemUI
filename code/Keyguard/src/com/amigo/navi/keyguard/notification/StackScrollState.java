...
public class StackScrollState {
    ...
    public void apply() {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (state == null) {
            	if(DebugLog.DEBUG) DebugLog.d(CHILD_NOT_FOUND_TAG, "No child state was found when applying this state " +
                        "to the hostView");
                continue;
            }
            if (!state.gone) {
                ...
                if (!( Math.abs(alpha - newAlpha) < 0.0000001 )&&/*alpha != newAlpha &&*/ xTranslation == 0) {
                    // apply layer type
                ...
                if (!( Math.abs(yTranslation - newYTranslation) < .0000001 )) {
                    child.setTranslationY(newYTranslation);
                }

                // apply zTranslation
                if (!( Math.abs(zTranslation - newZTranslation) < .0000001 )) {

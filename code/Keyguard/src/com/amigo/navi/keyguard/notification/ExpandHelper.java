...
//    private float mMaximumStretch;
    ...
    private OnScaleGestureListener mScaleGestureListener
        ...
        public boolean onScaleBegin(ScaleGestureDetector detector) {
        	if(DebugLog.DEBUG) DebugLog.v(TAG, "onscalebegin()");
        ...
        public boolean onScale(ScaleGestureDetector detector) {
        	if(DebugLog.DEBUG) DebugLog.v(TAG, "onscale() on " + mResizedView);
            return true;
        }
    ...
    private class ViewScaler {
        ...
        public void setHeight(float h) {
        	if(DebugLog.DEBUG) DebugLog.v(TAG, "SetHeight: setting to " + h);
...
//        mMaximumStretch = mSmallSize * STRETCH_INTERVAL;
    ...
    private void updateExpansion() {
    	if(DebugLog.DEBUG) DebugLog.v(TAG, "updateExpansion()");
    ...
    private boolean isInside(View v, float x, float y) {
    	if(DebugLog.DEBUG) DebugLog.d(TAG, "isinside (" + x + ", " + y + ")");

        if (v == null) {
        	if(DebugLog.DEBUG) DebugLog.d(TAG, "isinside null subject");
            return false;
        }
        if (mEventSource != null) {
            int[] location = new int[2];
            ...
            x += location[0];
            y += location[1];
            if(DebugLog.DEBUG) DebugLog.d(TAG, "  to global (" + x + ", " + y + ")");
        }
        int[] location = new int[2];
        v.getLocationOnScreen(location);
        x -= location[0];
        y -= location[1];
        if(DebugLog.DEBUG) DebugLog.d(TAG, "  to local (" + x + ", " + y + ")");
        if(DebugLog.DEBUG) DebugLog.d(TAG, "  inside (" + v.getWidth() + ", " + v.getHeight() + ")");
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(DebugLog.DEBUG) DebugLog.d(TAG, "intercept: act=" + MotionEvent.actionToString(action) +
        ...
        if(DebugLog.DEBUG) DebugLog.d(TAG, "set initial span: " + mInitialTouchSpan);
        ...
        if (mExpanding) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (xspan > mPullGestureMinXSpan &&
                	...
                	if(DebugLog.DEBUG) DebugLog.v(TAG, "got pull gesture (xspan=" + xspan + "px)");
                    startExpanding(mResizedView, PULL);
                    mWatchingForPull = false;
                }
                if (mWatchingForPull) {
                	final float yDiff = (ev.getRawY() - mInitialTouchY)*0.6f;
                    if (yDiff > mTouchSlop && ev.getY() > mInitialTouchY) {
                    	if(DebugLog.DEBUG) DebugLog.v(TAG, "got venetian gesture (dy=" + yDiff + "px)");
                        mWatchingForPull = false;
                    ...
                    }else if(yDiff > mTouchSlop  && ev.getY() < mInitialTouchY){
                        mWatchingForPull = false;
                        if (mResizedView != null && mCallback.canChildBeExpanded(mResizedView) 
                        		&& isFullyExpanded(mResizedView) && startExpanding(mResizedView, BLINDS)) {
                            mLastMotionY = ev.getRawY();
                            mInitialTouchY = ev.getRawY();
                            mHasPopped = false;
                        }
            ...
            case MotionEvent.ACTION_UP:
            	if(DebugLog.DEBUG) DebugLog.d(TAG, "up/cancel");
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if(DebugLog.DEBUG) DebugLog.d(TAG, "touch: act=" + MotionEvent.actionToString(action) +
                " expanding=" + mExpanding +
                (0 != (mExpansionStyle & BLINDS) ? " (blinds)" : "") +
                (0 != (mExpansionStyle & PULL) ? " (pull)" : "") +
                (0 != (mExpansionStyle & STRETCH) ? " (stretch)" : ""));
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
            	if(DebugLog.DEBUG) DebugLog.d(TAG, "pointer change");
    ...
    private boolean startExpanding(ExpandableView v, int expandType) {
        ...
        mExpanding = true;
        mCallback.expansionStateChanged(true);
        if(DebugLog.DEBUG) DebugLog.d(TAG, "scale type " + expandType + " beginning on view: " + v);
        mCallback.setUserLockedChild(v, true);
        mScaler.setView(v);
        mOldHeight = mScaler.getHeight();
        mCurrentHeight = mOldHeight;
        if (mCallback.canChildBeExpanded(v)) {
        	if(DebugLog.DEBUG) DebugLog.d(TAG, "working on an expandable child");
            mNaturalHeight = mScaler.getNaturalHeight(mLargeSize);
        } else {
        	if(DebugLog.DEBUG) DebugLog.d(TAG, "working on a non-expandable child");
            mNaturalHeight = mOldHeight;
        }
        if(DebugLog.DEBUG) DebugLog.d(TAG, "got mOldHeight: " + mOldHeight +
...
//        float h = mScaler.getHeight();
        ...
        if(DebugLog.DEBUG){
        DebugLog.d(TAG, "wasClosed is: " + wasClosed);
        DebugLog.d(TAG, "currentHeight is: " + currentHeight);
        DebugLog.d(TAG, "mSmallSize is: " + mSmallSize);
        DebugLog.d(TAG, "targetHeight is: " + targetHeight);
        DebugLog.d(TAG, "scale was finished on view: " + mResizedView);
        }
    }

...
public class StackStateAnimator {
    ...
    private void startAnimations(final ExpandableView child, StackScrollState.ViewState viewState,
        ...
        // order to modify findbugs
//        boolean yTranslationChanging = child.getTranslationY() != viewState.yTranslation;
        boolean yTranslationChanging = !(Math.abs(child.getTranslationY() - viewState.yTranslation) < 0.0000001);
        
//        boolean zTranslationChanging = child.getTranslationZ() != viewState.zTranslation;
        boolean zTranslationChanging = !(Math.abs(child.getTranslationZ() - viewState.zTranslation) < 0.0000001);
        boolean scaleChanging = child.getScaleX() != viewState.scale;
        boolean alphaChanging = /*alpha != child.getAlpha();*/!( Math.abs(alpha - child.getAlpha()) < .0000001 );
...
//        }
        ...
        for (NotificationStackScrollLayout.AnimationEvent event : mNewEvents) {
            ...
            switch (event.animationType) {
                ...
                case NotificationStackScrollLayout.AnimationEvent.ANIMATION_TYPE_REMOVE_SWIPED_OUT:
                    ...
                    minDelay = removeAnimation(viewState, finalState, minDelay,
							event, delayPerElement);
                    break;
	...
	private long removeAnimation(StackScrollState.ViewState viewState,
			StackScrollState finalState, long minDelay,
			NotificationStackScrollLayout.AnimationEvent event,
			long delayPerElement) {
		int ownIndex = viewState.notGoneIndex;
	...
	private void startAlphaAnimation(final ExpandableView child,
        ...
        if (previousEndValue != null && ( Math.abs(previousEndValue - newEndValue) < .0000001 )) {
            return;
        }

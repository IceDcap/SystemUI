...
public class StackScrollAlgorithm {
    ...
    private static final int MAX_ITEMS_IN_BOTTOM_STACK = 1;
    private static final int MAX_ITEMS_IN_TOP_STACK = 1;

    public static final float DIMMED_SCALE = 0.95f;
    private static final float ZDISTANCE_FACTOR_OF_ACTIVATED = 6.0f;
...
//                .getDimensionPixelSize(R.dimen.notification_min_height);
        ...
        mTopStackSlowDownLength = 0;//context.getResources()
//                .getDimensionPixelSize(R.dimen.top_stack_slow_down_length);
        ...
        mScaleDimmed = false;//context.getResources().getDisplayMetrics().densityDpi
//                >= DisplayMetrics.DENSITY_XXHIGH;
    ...
    private void updateClipping(StackScrollState resultState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if(state.location == StackScrollState.ViewState.LOCATION_TOP_STACK_PEEKING
            		|| state.location == StackScrollState.ViewState.LOCATION_TOP_STACK_HIDDEN){
            	state.clipTopAmount = 0;
            	state.topOverLap = 0;
            }
            if(state.topOverLap != 0 ){
            	state.clipTopAmount = state.topOverLap;
            }
    ...
    private void updateDimmedActivatedHideSensitive(AmbientState ambientState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (dimmed && isActivatedChild) {
                childViewState.zTranslation += ZDISTANCE_FACTOR_OF_ACTIVATED * mZDistanceBetweenElements;
            }
        }
    }
...
//            }
            if (childViewState.location == StackScrollState.ViewState.LOCATION_UNKNOWN) {
            	if(DebugLog.DEBUG) DebugLog.d(LOG_TAG, "Failed to assign location for child " + i);
            }
    ...
    private void updateStateForTopStackChild(StackScrollAlgorithmState algorithmState,
        ...
        int paddedIndex = i - 1
                - Math.max(numberOfElementsCompletelyIn - MAX_ITEMS_IN_TOP_STACK, -1);
        if (paddedIndex >= 0) {
...
//                    }
    ...
    public float getOffsetOfTopChild(){
    	...
    	int height = child!=null?getMaxAllowedChildHeight(child):0;
    	float partial = mTempAlgorithmState.partialInTop>=0.5?
    			(1-mTempAlgorithmState.partialInTop):-mTempAlgorithmState.partialInTop;
    	return partial*height;
    }

    static class StackScrollAlgorithmState {

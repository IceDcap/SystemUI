...
import com.amigo.navi.keyguard.modules.KeyguardMissedInfoModule;
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private KeyguardNotificationModule mNotificationModule=null;
    private KeyguardMissedInfoModule mMissedInfoModule = null;
    private WeatherInfoModule mWeatherInfoModule=null;
    private ArrayList<KeyguardModuleBase> mModules = Lists.newArrayList();
    
    // guosb add for battery begin
    // Battery status
    private AmigoBatteryStatus mAmigoBatteryStatus;
    // guosb add for battery end
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SIM_SUBSCRIPTION_INFO_CHANGED:
                    ...
                    break;
                case MSG_MISSED_CALL_COUNT_CHANGED:
                    mMissedInfoModule.handleMissedCallCountChanged(msg.arg1);
                    break;
                case MSG_MISSED_MMS_COUNT_CHANGED:
                    mMissedInfoModule.handleMissedMmsCountChanged(msg.arg1);
                    break;
                case MSG_MISSED_SMS_INFO_GONE:
                    mMissedInfoModule.handleMissedSmsInfoCount();
                    break;
            }
        }
    };
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                // guosb add for Batteryinfo begin
                ...
                Message msg = mHandler.obtainMessage(
                        MSG_BATTERY_UPDATE, new AmigoBatteryStatus(status, level, plugged, health));
                mHandler.sendMessage(msg);
                // guosb add for Batteryinfo end
    ...
    private KeyguardUpdateMonitor(Context context) {
        ...
        // guosb add for battery begin
        // Take a guess at initial SIM state, battery status and PLMN until we get an update
        mAmigoBatteryStatus = new AmigoBatteryStatus(BATTERY_STATUS_UNKNOWN, 100, 0, 0);
        // guosb add for battery end
    ...
    private void initModules(Context context) {
      mMissedInfoModule = KeyguardMissedInfoModule.getInstance(context, this);
      mWeatherInfoModule=WeatherInfoModule.getInstance(context, this);
      mNotificationModule = KeyguardNotificationModule.getInstance(mContext,this);
      mModules.add(mMissedInfoModule);
      mModules.add(mWeatherInfoModule);
      mModules.add(mNotificationModule);
      
      for(KeyguardModuleBase m : mModules) {
          m.registerReceiver();
      }
  }
    ...
    public void activeQueryMissedCallCount() {
        mMissedInfoModule.startMissedCallQuery();
    }
    
    public void activeQueryMissedMmsCount() {
        mMissedInfoModule.startMissedMmsQuery();
    }
  
    public void notifyMissedCallCountChanged(int count) {
        mHandler.obtainMessage(MSG_MISSED_CALL_COUNT_CHANGED, count, 0).sendToTarget();
    }
    
    public void notifyMissedMmsCountChanged(int count) {
        mHandler.obtainMessage(MSG_MISSED_MMS_COUNT_CHANGED, count, 0).sendToTarget();
    }
    public void notifyMissedSmsInfoGone() {
        mHandler.obtainMessage(MSG_MISSED_SMS_INFO_GONE, 0, 0).sendToTarget();
    }
    
    public void registerMissedInfoCallback(MissedInfoCallback callback) {
        DebugLog.d(TAG, "MissedInfoCallback registered");
        mMissedInfoModule.registerCallback(callback);
    }
    
    public void unregisterMissedInfoCallback(MissedInfoCallback callback) {
        DebugLog.d(TAG, "MissedInfoCallback unregistered");
        mMissedInfoModule.unregisterCallback(callback);
    }
    
  public void registerWeatherInfoCallback(WeatherInfoUpdateCallback callback){
      DebugLog.d(TAG, "WeatherInfoCallback registered");
      mWeatherInfoModule.registerCallback(callback);
  }
  public void unregisterWeatherInfoCallback(WeatherInfoUpdateCallback callback){
      DebugLog.d(TAG, "WeatherInfoCallback registered");
      mWeatherInfoModule.unregisterCallback(callback);
  }
     ...
     */
    private void handleBatteryUpdate(AmigoBatteryStatus status) {
        if (DEBUG) Log.d(TAG, "handleBatteryUpdate");
//        final boolean batteryUpdateInteresting = isBatteryUpdateInteresting(mBatteryStatus, status);
//        mBatteryStatus = status;
        final boolean batteryUpdateInteresting = isBatteryUpdateInteresting(mAmigoBatteryStatus, status);
        if (DEBUG) Log.d(TAG, "handleBatteryUpdate batteryUpdateInteresting："+batteryUpdateInteresting);
        mAmigoBatteryStatus = status;
    ...
    private static boolean isBatteryUpdateInteresting(AmigoBatteryStatus old, AmigoBatteryStatus current) {
    ...
    private void sendUpdates(KeyguardUpdateMonitorCallback callback) {
        ...
        callback.onRefreshBatteryInfo(mAmigoBatteryStatus);
    ...
    //guosb add for notification begin
    public KeyguardNotificationModule getNotificationModule(){
        return mNotificationModule;
    }
    
    public void showNotificationClickTip(boolean isShow,com.amigo.navi.keyguard.notification.ActivatableNotificationView view){
        DebugLog.d(TAG, "showNotificationClickTip(), isShow=" + isShow);
        for(int i = 0, n = mCallbacks.size(); i < n; ++i) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if(cb != null) {
                cb.onNotificationClickTipShow(isShow,view);
            }
        }
    }
    
    public void resetNotificationClick(com.amigo.navi.keyguard.notification.ActivatableNotificationView view){
        DebugLog.d(TAG, "resetNotificationClick " + view);
        for(int i = 0, n = mCallbacks.size(); i < n; ++i) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if(cb != null) {
                cb.onResetNotificationClick(view);
            }
        }
    }

        ...
        if (mExpanding) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mWatchingForPull) {
                    ...
                    final float xDiff = Math.abs(ev.getRawX() - mInitialTouchX);
                    if (yDiff > mTouchSlop) {
                    ...
                    // Let's single figure-touching collapse notification possible
					} else if (xDiff < mTouchSlop && yDiff < -5f) {
						if (mResizedView != null
								&& (mResizedView instanceof ExpandableNotificationRow)) {
							if (((ExpandableNotificationRow) mResizedView)
									.isExpanded()) {
								mLastMotionY = ev.getRawY();
                                mInitialTouchY = ev.getRawY();
                                mInitialTouchX = ev.getRawX();
